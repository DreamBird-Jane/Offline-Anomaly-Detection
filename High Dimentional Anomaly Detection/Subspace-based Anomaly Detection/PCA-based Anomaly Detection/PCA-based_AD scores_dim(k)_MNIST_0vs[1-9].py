...
## Define abnormal digit: d_ab = 10 means digits [1-9] are abnormal
d_ab = 10      #<----------------------------------------- you can define number as anomalies other than d_normal
N_abs = [34, 34, 34, 33, 33, 33, 33, 33, 33] #sample size for each digit 1-9
N_ab = 300 #sum(N_abs)
...
for it in range(n_iter):
    ...
    ## Random select abnormal data 1~9    
    Train_X_ab_sel = pd.DataFrame([],columns = Train_X.columns)
    Train_y_ab = pd.DataFrame([],columns = ['Is_Ab'])
    
    for i in range(1,d_ab): 
        ...
        df_y_ab = pd.DataFrame(np.ones(N_abs[i-1]), columns=['Is_Ab'])#Train_y[Train_y.values == i][:N_abs[i-1]].copy()  
        df_y_ab.index = df_X_ab.index
        
        Train_X_ab_sel = Train_X_ab_sel.append(df_X_ab)
        Train_y_ab = Train_y_ab.append(df_y_ab)
    ...
    print("Frequency of unique values of the said array:")
    print( np.asarray((unique_elements, counts_elements)) )
    del Train_X_0_sel, Train_X_ab_sel, Train_y_0, Train_y_ab, indices_select, df_X_ab, df_y_ab
